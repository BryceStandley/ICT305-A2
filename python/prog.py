...
from bokeh.models import HoverTool, ColumnDataSource, Div, Slider, Range1d, PrintfTickFormatter, InlineStyleSheet, \
    Button, CustomJS, SetValue, TabPanel, Tabs, Dropdown, CheckboxGroup
...
# Trim Data
df_trim = all_cities_df.loc[:,
          ('Date', 'city', 'day', 'month', 'year', 'Evaporation (mm)', 'Rainfall (mm)', 'Sunshine (hours)')]
...
data2 = df_trim[(df_trim['year'] == default_year) & (df_trim['month'] == default_month) & (df_trim['city'] == default_sub_capital)]
...
evapPlot = CreateLinePlot(currentlySelectedMainCapital.capitalize() + ' vs ' + currentlySelectedSubCapital.capitalize() +" Evaporation in {month} {year}", df_trim, trimmedMainSourceData, trimmedSubSourceData, default_month,
                          default_year, "x", "evap", "Day of the Month", "Evaporation")
rainPlot = CreateLinePlot(currentlySelectedMainCapital.capitalize() + ' vs ' + currentlySelectedSubCapital.capitalize() + " Rainfall in {month} {year}", df_trim, trimmedMainSourceData, trimmedSubSourceData, default_month, default_year,
                          "x", "rain", "Day of the Month", "Rainfall")
sunPlot = CreateLinePlot(currentlySelectedMainCapital.capitalize() + ' vs ' + currentlySelectedSubCapital.capitalize() + " Sunshine in {month} {year}", df_trim, trimmedMainSourceData, trimmedSubSourceData, default_month, default_year,
                         "x", "sun", "Day of the Month", "Sunshine")
...
def update_plots(yearValue, monthValue, capitalMain, capitalSub):
    ...
    d = df_trim[(df_trim['year'] == yearValue) & (df_trim['month'] == monthValue) & (df_trim['city'] == capitalMain)].reset_index(drop=True)
    d2 = df_trim[(df_trim['year'] == yearValue) & (df_trim['month'] == monthValue) & (df_trim['city'] == capitalSub)].reset_index(drop=True)
...
janTempsTab = TabPanel(child=janTemps, title='January 2021')
monthlyPlotTempsTab = TabPanel(child=monthly_Plot_layout, title='Monthly')
monthlyAvgPlotTempsTab = TabPanel(child=monthly_avg_temp_layout, title='Monthly Avg Temperatures')
...
tempTabs = TabPanel(child=Tabs(
    tabs=[janTempsTab, monthlyPlotTempsTab, monthlyAvgPlotTempsTab, temp_YearOverYear_plotTempsTab,
          monthly_MinMax_plot_layoutTempsTab, yearly_MinMax_plot_layoutTempsTab], margin=(10, 0, 0, 0)),
                    title='Temperature')
